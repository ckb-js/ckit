...
function isPwLock(script: Script, config: Config) {
  const template = config.SCRIPTS.PW_NON_ANYONE_CAN_PAY!;
...
export async function setupInputCell(
  ...
  if (!isPwLock(fromScript, config)) {
    throw new Error(`Not PW_NON_ANYONE_CAN_PAY input!`);
  }
  ...
  const template = config.SCRIPTS.PW_NON_ANYONE_CAN_PAY;
  if (!template) {
    throw new Error(`PW_NON_ANYONE_CAN_PAY script not defined in config!`);
  }
...
export function prepareSigningEntries(
  ...
  console.log('pw prepare');
  config = config || getConfig();

  const template = config.SCRIPTS.PW_NON_ANYONE_CAN_PAY;
  if (!template) {
    throw new Error(`Provided config does not have PW_NON_ANYONE_CAN_PAY script setup!`);
  }
